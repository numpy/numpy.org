...
- 기본 네임스페이스의 유형 주석은 기본적으로 완전합니다. Upstream is
...
- The universal functions have been refactored to implement most of
  ...
  이것은 또한 미래의 DType API를 실험할 수 있는 능력을 잠금 해제합니다.
- 다운스트림 프로젝트에서 사용할 새로운 구성 가능한 메모리 할당자입니다.
...
communities. 이 프로젝트는 Melissa Mendonça(NumPy) 가 이끌고 Ralf Gommers(NumPy, SciPy), Hannah Aizenman 및 Thomas Caswell(Matplotlib), Matt Haberland(SciPy), Joris Van den Bossche(Pandas) 가 추가 멘토링 및 지침을 제공합니다.

이것은 프로젝트의 커뮤니티 역학을 구조적으로 개선해야 하는 활동을 발견하고 구현하는 것을 목표로 하는 야심 찬 프로젝트입니다. 이러한 새로운 교차 프로젝트 역할을 설정함으로써 우리는 Scientific Python 커뮤니티에 새로운 협업 모델을 도입하여 생태계 내에서 커뮤니티 구축 작업을 보다 효율적으로 수행하고 더 큰 결과를 얻을 수 있기를 바랍니다. 우리는 또한 특히 역사적으로 과소대표된 그룹의 새로운 기여자를 참여시키고 유지하기 위해 프로젝트에서 효과가 있는 것과 그렇지 않은 것에 대한 명확한 그림을 개발할 것으로 기대합니다. 마지막으로, 우리는 실행된 조치에 대한 자세한 보고서를 작성하여 커뮤니티와의 대표 및 상호 작용 측면에서 프로젝트에 어떤 영향을 미쳤는지 설명할 계획입니다.
...
is now available. 역대 최대의 NumPy 릴리즈입니다. 180명이 넘는 기여자분들께 감사드립니다. 흥미롭고 새로운 두 기능이 나왔습니다.
...
as a review article in Nature. 넘파이 1.0이 나온 지 14년 만입니다.
